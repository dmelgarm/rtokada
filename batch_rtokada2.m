...
%runName='RTOkada_HiResW_kal_gps_LCopt'
...
lambda=logspace(-2,log10(20),200);
weightflag=1;
%lambda=logspace(-2,3,50)
%lambda=logspace(-3,5,30);
%lambda=50;
N=length(lambda);
cd(workpath)
% load('green.mat')
...
for k=1:N
    k
    %tic
    [l(k) L2(k) LS(k) Mo(k) Mw(k) VR(k) GCV(k) ABIC(k)]=rtokada2(workpath,outpath,runName,runID{k},stasuffix,lambda(k),Gp,Gs,weightflag,stations);
    %toc
end
toc
for k=1:N
